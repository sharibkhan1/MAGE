...
import { useMenuOptionsAdmin } from '@/lib/constants';
...
const MenuOptionsAdmin = (props: Props) => {
  const pathName = usePathname();
  const menuOptions = useMenuOptionsAdmin(); // Use the hook to get menu options
          ...
          {menuOptions.map((menuItem) => (
            <ul key={menuItem.name}>
              <Tooltip delayDuration={0}>
                ...
                <TooltipContent side="right" className="bg-black/10 text-white backdrop-blur-xl">
                  <p>{menuItem.name}</p>
                </TooltipContent>
              </Tooltip>
            </ul>

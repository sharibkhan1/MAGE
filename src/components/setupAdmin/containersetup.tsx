...
const onSubmit: SubmitHandler<z.infer<typeof VectorModelSchema>> = async (data) => {
  ...
  return (
      <ShineBorder className="w-[80%] h-[95%] bg-black  shadow-md dark:text-black rounded-2xl relative" color={["white", "#d3d4d3", "#853F67"]}>
        <DotPattern
                  className={cn(
                    "[mask-image:radial-gradient(1400px_circle_at_center,white,transparent)]"
                  )}
                />
        <div className="absolute top-0 leftl-0 right-0 ">
          <Progress value={progress} />
        </div>
        {step === 1 ? (
          <Stepone onNext={handleNextStep} step={step} />
        ): step === 2 ? (
          <div className="flex z-[2] flex-row w-[100%]  h-[100%] " >
            <div className="flex flex-col mt-[5rem] min-h-[50rem] w-[20rem] space-y-4">
              <Button variant="outline" className={step === 2 ? "px-4 py-0.5 ethereal-chill dark:text-white  border-2 border-black dark:border-white uppercase  text-black transition duration-200 text-sm shadow-[1px_1px_rgba(0,0,0),2px_2px_rgba(0,0,0),3px_3px_rgba(0,0,0),4px_4px_rgba(0,0,0),5px_5px_0px_0px_rgba(0,0,0)] dark:shadow-[1px_1px_rgba(255,255,255),2px_2px_rgba(255,255,255),3px_3px_rgba(255,255,255),4px_4px_rgba(255,255,255),5px_5px_0px_0px_rgba(255,255,255)]" : ""}>
                Vector Index
              </Button>
              <Button variant="outline" className="text-white px-4 py-0.5 border-white" disabled={isModelConfigured}>
              Embedding Models
        </Button>
        <Button variant="outline" disabled={isModelConfigured} className="text-white border-white px-4 py-0.5">
        Data Loaders
        </Button>
            </div>

            <div className="flex-1  z-[10] p-8 rounded-lg ml-4 ">
              ...
              <div className="grid grid-cols-2 overflow-y-auto max-h-[60vh] gap-5">
    ...
    <div onClick={() => setSelectedModel(model)}>
      <CardContainer>  
        <CardBody
          className={`bg-black mt-5 w-[250px] h-[250px] group/card dark:hover:shadow-2xl dark:hover:shadow-emerald-500/[0.1] shadow-lg rounded-lg flex flex-col justify-center items-center cursor-pointer border border-[#ffd078ab] ${isModelConfigured ? " opacity-70 cursor-not-allowed" : ""}`}
          style={{
        ...
        >
          ...
          <CardItem
            as="p"
            translateZ="60"
            className="text-center mt-4 text-white text-2xl border-t-2 border-[#ffd078ab] w-[50%] font-semibold"
          >
            {model}
          </CardItem>
        </CardBody>
        ...
        {selectedModel === "Pinecone" && (
          ...
          />
                          ...
                          <Button onClick={handleBackStep}  variant="outline">Back</Button>
                            ...
                            <Button type="submit" className=" ml-4" variant="default" disabled={isModelConfigured}>
                              Submit
                            </Button>
                          </div>
                        </form>

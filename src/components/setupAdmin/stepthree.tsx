...
const StepThree: React.FC<StepTwoProps> = ({ onBack, onNext, step }) => {
...
const sourceMap = {
  AWS: { source: "aws", file_type: "all" },
  Azure: { source: "azure", file_type: "all" },
  Google: { source: "google_drive", file_type: "all" },
  URL_loader: { source: "url", file_type: "all" },
  Notion: { source: "notion", file_type: "pages" },
} as const;

type ModelType = keyof typeof sourceMap;
...
const onSubmit: SubmitHandler<z.infer<typeof DataLoaderModelSchema>> = async (data) => {
  ...
  // Type guard to ensure selectedModel is a valid key
  if (!Object.keys(sourceMap).includes(selectedModel)) {
  ...
  // Use selectedModel which is guaranteed to be a valid key
  const { source, file_type } = sourceMap[selectedModel as ModelType];

  try {
    const response = await fetch("http://localhost:8000/docs/api/loading", {
      method: "POST",
      headers: {
        "Content-Type": "application/x-www-form-urlencoded",
      },
      body: new URLSearchParams({ source, file_type }),
    });
    ...
    if (response.ok) {
      ...
      setError("Error loading documents: " + loadResult.error);
    }
  } catch (error) {
  ...
  return (
<div className="flex flex-row w-[100%] h-[100%] overflow-hidden " >
<div className="flex flex-col mt-[5rem] min-h-[50rem] w-[20rem] space-y-4">
        ...
        <Button variant="outline" className={step === 4 ? "px-8 py-0.5 z-[4] ethereal-chill dark:text-white  border-2 border-black dark:border-white uppercase text-black transition duration-200 text-sm shadow-[1px_1px_rgba(0,0,0),2px_2px_rgba(0,0,0),3px_3px_rgba(0,0,0),4px_4px_rgba(0,0,0),5px_5px_0px_0px_rgba(0,0,0)] dark:shadow-[1px_1px_rgba(255,255,255),2px_2px_rgba(255,255,255),3px_3px_rgba(255,255,255),4px_4px_rgba(255,255,255),5px_5px_0px_0px_rgba(255,255,255)]" : ""}>
          Data Loaders
        </Button>
      </div>

      <div className="flex-1 p-8 rounded-lg ml-4 ">  {/* Set max-height and overflow for scrolling */}
      <h3 className="text-2xl mb-4 pointer-events-none whitespace-pre-wrap bg-gradient-to-b from-black to-gray-400 bg-clip-text text-center font-semibold leading-none text-transparent dark:from-white dark:to-[#928a92]">Choose an Embedding Model</h3>
        <div className="grid grid-cols-2 gap-4 overflow-x-hidden overflow-y-auto max-h-[60vh]">
      ...
      <CardContainer>  
        <CardBody
          className={`bg-black mt-5 w-[250px] h-[250px] group/card dark:hover:shadow-2xl dark:hover:shadow-emerald-500/[0.1] shadow-lg rounded-lg flex flex-col justify-center items-center cursor-pointer border border-[#ffd078ab] ${isModelConfigured ? "opacity-50 cursor-not-allowed" : ""}`}
          style={{
            boxShadow: "inset 0 0 40px 1px #ffd078ab, inset 0 -2px 0 0 transparent",
          }}
        >
          {/* Card Image */}
          <CardItem
          ...
          <CardItem
            as="p"
            translateZ="60"
            className="text-center w-[50%] text-white border-t-2 border-[#ffd078ab] mt-4 text-lg font-semibold"
          >
            {model}
          </CardItem>
        </CardBody>

...
import FinalSteps from "./cong";
import Steone from "./setone";
import SteTwo from "./settwo";
import SteThree from "./setthree";
...
export default function SetupContainerers() {
...
const onSubmit: SubmitHandler<z.infer<typeof VectorModelSchema>> = async (data) => {
  ...
  return (
      <ShineBorder className="w-[90rem] bg-muted dark:bg-muted p-8 shadow-md dark:text-black rounded-lg relative" color={["white", "#d3d4d3", "black"]}>
        ...
        {step === 1 ? (
          <Steone onNext={handleNextStep} step={step} />
        ): step === 2 ? (
    ...
    <form onSubmit={form.handleSubmit(onSubmit)}>
      ...
      {(selectedModel === "Pinecone" || selectedModel === "Faiss")&& (
        ...
        {selectedModel === "Pinecone" && (
          ...
          <SteTwo onBack={handleBackStep} onNext={handleNextStep} step={step} />
        ) : step === 4 ? (
          <SteThree onBack={handleBackStep} onNext={handleNextStep} step={step} />
        ): (
          <FinalSteps
            onReset={handleReset}
